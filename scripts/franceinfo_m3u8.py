...
# URL source
url = "https://raw.githubusercontent.com/Paradise-91/ParaTV/main/streams/francetv/franceinfo.m3u8"

# Dossier et fichier de sortie
FHD_OUTPUT_DIR = "./Streams"
FHD_OUTPUT_FILE = "franceinfo.m3u8"
...
try:
    ...
    if response.status_code == 200:
        ...
        # Extraction des URLs contenant '2.m3u8'
        urls_m3u8 = re.findall(r'(https?://[^\s]+2\.m3u8)', contenu)
        urls_sans_suffixe = [re.sub(r'2\.m3u8$', '', url) for url in urls_m3u8]
        ...
        if urls_sans_suffixe:
            ...
            fhd_content = generate_m3u_content(m3u8_url, "1280x720", 4277000, "2.m3u8", "1.m3u8")
            ...
            log("❌ Aucune URL contenant '2.m3u8' trouvée.")
            sys.exit(1)
    else:
